...
def red_button(board: Board):
    for game_object in board.game_objects:
        if (game_object.type == "DiamondButtonGameObject"):
            return game_object.position

def best_diamond(board_bot: GameObject, board: Board):
...
class BestDiamondLogic(BaseLogic):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        #     self.goal_position = tackle_nearest_enemy(board_bot, board)
            ...
            d_red_button = distance(red_button(board), current_position)
            d_best_diamond = distance(best_diamond(board_bot, board), current_position)

            if (d_red_button < d_best_diamond):
                self.goal_position = red_button(board)
            else:
                self.goal_position = best_diamond(board_bot, board)
